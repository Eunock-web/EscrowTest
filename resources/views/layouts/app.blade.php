        ...
        <nav class="flex-1">
            @auth
                @if(Auth::user()->role === 'createur')
                    ...
                    <a href="{{ route('creator.escrow') }}" class="nav-link {{ request()->routeIs('creator.escrow') ? 'active' : '' }}">
                        ...
                        <span class="sidebar-text flex items-center gap-2">
                            Escrow
                            @php
                                $pendingCount = \App\Models\Sale::where('seller_id', Auth::id())->where('status', 'escrow_locked')->count();
                            @endphp
                            @if($pendingCount > 0)
                                <span class="ml-auto px-1.5 py-0.5 bg-amber-500 text-white text-[9px] font-bold rounded-full leading-none sidebar-text">{{ $pendingCount }}</span>
                            @endif
                        </span>
                    </a>
                @elseif(Auth::user()->role === 'admin')
                    ...
                    <a href="{{ route('admin.analytics') }}" class="nav-link {{ request()->routeIs('admin.analytics') ? 'active' : '' }}">
                        <svg class="w-5 h-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 19v-6a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2a2 2 0 002-2zm0 0V9a2 2 0 012-2h2a2 2 0 012 2v10m-6 0h6m2 0h2a2 2 0 002-2v-4a2 2 0 00-2-2h-2a2 2 0 00-2 2v4a2 2 0 002 2z"/></svg>
                        <span class="sidebar-text">Analytics Global</span>
                    </a>
